...
router.post("/jobvecancysingledata", async (req, res) => {
  ...
  const singledata = await dbs
    .collection("jobvecancy")
    .findOne({ _id: new ObjectId(bodyid) });
...
router.put("/updatejobapplicationdata", async (req, res) => {
  ...
  const updatedcollection = await dbs.collection("jobvecancy").updateOne(
...
router.delete("/deletejobapplicationdata", async (req, res) => {
  const bodydata = req.body;
  const deletecollection = await dbs
    .collection("jobvecancy")
    .deleteOne({ _id: new ObjectId(bodydata.id) });
